...
def luminance(block):
    ...
    matrix_luminance = eval(matrix)
    
    blockq = np.around(np.divide(block, matrix_luminance))
    blockq_inv = np.around(np.multiply(blockq, matrix_luminance))
...
def main():
    ...
    # Calculate the luminance for each block
    lum_res = []
    for l in dct_res:
        lum_res.append(luminance(l))
